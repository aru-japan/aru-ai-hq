...
**Relation設定件数の内訳（2026-07-20 読み取り専用再確認により追記）**：既存Research／Articlesと意味的に重複していた元テーマは6問（#3, #4, #6, #10, #13, #14）。うち#14は銀行口座・携帯電話で必要書類が異なるため14-A・14-Bへ分割登録したため、Relationを設定したQAカードのレコード数は**7件**（#3, #4, #6, #10, #13, #14-A, #14-B）となる。内訳：Research・Article双方に接続＝5件（#3, #6, #13, #14-A, #14-B）、Researchのみ接続（対応記事が未生成のため）＝2件（#4, #10）。残り9件（#1, #2, #5, #7, #8, #9, #11, #12, #15）はRelationなし。
