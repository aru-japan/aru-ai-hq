...
`a5ea29d`（このHandover文書を書いた時点でのHEAD）。**必ず`git log --oneline -10`で実際の最新を確認すること。** このフィールドは経年劣化する。
...
- **手動Dashboard（13 Linked View）の並べ替えが未実施**（2026-07-19）：[Dashboard-Setup-Guide.md](./Dashboard-Setup-Guide.md)「ARu Studio v4.1 推奨並び順」に従い、Rei自身がNotion上で既存6ブロックのドラッグ&ドロップ並べ替え＋新規2種類（今日追加するQA＝Story Bank、今日作る記事＝Articles Production Stage別）のLinked View追加を行う必要がある。AI Command Center側はコードで既に反映済み
