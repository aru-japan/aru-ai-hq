...
- **Production Stage（Story Bank新規プロパティ、2026-07-19）** `notion-build/add_production_stage.py`：Reiが提示した1つのStoryの制作パイプライン（Today's QA→Headline Ready→Basic Writing→Deep Writing→Translation→SNS→Ready→Published）をStory Bankの新規Select（15→32プロパティ、Stage 2のリレーション3件込み）として追加。既存のStory Status（粗い編集トリアージ）・Content Type（コンテンツの種類分類）とは意図的に別軸。スキーマのみでバックフィルなし——既存21件はQAカード化未実施の実データのため、架空の段階を割り当てず空欄のまま（捏造禁止ルール）。自動進行の自動化は未実装（要判断）
...
`7a6cca9`（このHandover文書を書いた時点でのHEAD）。**必ず`git log --oneline -10`で実際の最新を確認すること。** このフィールドは経年劣化する。
